...
/**
 * Returns a function that infers properties of TypeAlias objects
 * (Flow or TypeScript type definitions) given a string with a pattern that might
 * infer private access level, like `^_`. That access attribute is not standard.
 *
 * @param {?string} pattern regexp-compatible pattern
 * @returns {Function} inference method
 * @private
 */
function inferPropertiesWithPattern(pattern/*: ?string*/) {
   ...
   */
  return function(comment/*: Comment */)/*: Comment */ {
    ...
    function inferProperties(value, prefix) {
      if (value.type === 'ObjectTypeAnnotation') {
        value.properties.forEach(function (property) {
          var inferedPropertyDoc = propertyToDoc(property, prefix);
          const shouldIgnoreProperty = re !== undefined && re.test(property.key.name);
          if (shouldIgnoreProperty) {
            inferedPropertyDoc.access = 'private';
          }
          // Nested type parameters
          if (property.value.type === 'ObjectTypeAnnotation') {
            inferProperties(property.value, prefix.concat(property.key.name));
          }
          var propertyName = prefixedName(property.key.name, prefix);
...
module.exports = inferPropertiesWithPattern;
